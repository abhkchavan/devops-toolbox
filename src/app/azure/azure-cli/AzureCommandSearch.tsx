...
export default function AzureCommandSearch({ sections }: Props) {
  ...
  const filteredSections = useMemo(() => {
    ...
    return sections
      ...
      .filter(
        (section): section is AzureSection => section !== null,
      );
  }, [search, sections]);
    ...
    <div className="mb-10">
      <div className="flex flex-col gap-3 sm:flex-row">
        <input
          ...
          placeholder="Search Azure CLI commands..."
      ...
      {search && filteredSections.length === 0 && (
        <div className="mt-6 rounded-lg border border-slate-800 bg-slate-900 p-6 text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Try searching for VM, Storage, VNet, AKS, ACR, App
            Service, Functions, SQL, Key Vault, IAM or Monitor.
          </p>
        </div>
      )}
