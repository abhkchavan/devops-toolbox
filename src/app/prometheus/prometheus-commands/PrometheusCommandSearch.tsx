...
export default function PrometheusCommandSearch({
  sections,
}: Props) {
  ...
  const filteredSections = useMemo(() => {
    ...
    return sections
      ...
      .filter(
        (section): section is PrometheusSection =>
          section !== null,
      );
  }, [query, sections]);
  ...
  return (
    <div className="mb-10">
        ...
        <section className="mb-10">
          <div className="mb-5">
            <h2 className="text-2xl font-bold text-white">
              Prometheus Command Categories
            </h2>

            <p className="mt-2 text-slate-400">
              Jump directly to the Prometheus and PromQL commands
              you need.
            </p>
          </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <input
          id="prometheus-search"
          type="text"
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder="Search Prometheus commands or PromQL..."
      ...
      {query && filteredSections.length === 0 && (
        <div className="mt-6 rounded-lg border border-slate-800 bg-slate-900 p-6 text-center">
          <p className="font-semibold text-white">
            No Prometheus commands found
          </p>

          <p className="mt-2 text-sm text-slate-400">
            Try searching for PromQL, promtool, alert, recording,
            target, scrape, rate, histogram, CPU, memory, disk,
            Kubernetes, API or troubleshooting.
          </p>
